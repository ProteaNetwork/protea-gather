...
const makeSelectFilteredMyUpcomingEvents = createSelector(selectMyEventsFilter, selectMyUpcomingEvents,
  (filter, events) => {
  return events.filter(event => (filter == "" || (event && event.name &&  event.name.toLowerCase().indexOf(filter.toLowerCase()) >= 0)))
})
...
const makeSelectFilteredMyPendingClaimEvents = createSelector(selectMyEventsFilter, selectMyEvents, makeSelectEthAddress,
  (filter, events, ethAddress) => {
    return events
      .filter(event => (((event.state == 3 && event.memberState == 99) || (event.state == 4 && event.memberState == 1 && event.gift > 0)) && event.organizer != ethAddress))
      .filter(event => (filter == "" || (event && event.name && event.name.indexOf(filter) >= 0)))
})

const makeSelectFilteredMyPendingConfirmEvents = createSelector(selectMyEventsFilter, selectMyEvents,
  (filter, events) => {
  return events.filter(event => (event.state == 2 && event.memberState == 1))
  .filter(event => (filter == "" || (event && event.name &&  event.name.toLowerCase().indexOf(filter.toLowerCase()) >= 0)))
})
