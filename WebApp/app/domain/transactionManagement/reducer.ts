...
import { setBalancesAction, setRemainingTxCountAction, setTxContextAction, setCommunityMutexAction, setQrAction, setTxPendingState } from './actions';
...
function transactionManagementReducer(state: DomainState = initialState, action: DomainActions ) {
  switch (action.type) {
    ...
    case getType(setTxPendingState):
      return{
        ...state,
